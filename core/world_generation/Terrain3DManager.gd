	...
	if ResourceLoader.exists(TERRAIN_CONFIG_PATH):
		...
		if file:
			...
			if parsed is Dictionary:
				...
				MythosLogger.debug("World/Terrain", "Config loaded", {"data_directory": data_directory})
			else:
				MythosLogger.warn("World/Terrain", "Config file exists but contains invalid JSON")
		else:
			MythosLogger.warn("World/Terrain", "Failed to open config file for reading")
	else:
		MythosLogger.debug("World/Terrain", "Config file not found, creating default")
		...
		if file:
			...
			file.close()
			MythosLogger.info("World/Terrain", "Created default config file")
		else:
			MythosLogger.error("World/Terrain", "Failed to create default config file")

func create_terrain() -> void:
	MythosLogger.verbose("World/Terrain", "create_terrain() called")
	if terrain:
		MythosLogger.debug("World/Terrain", "Existing terrain found, freeing")
		terrain.queue_free()

	terrain = ClassDB.instantiate("Terrain3D")
	if not terrain:
		MythosLogger.error("World/Terrain", "FATAL: Terrain3D GDExtension failed to load!")
		return
	...
	MythosLogger.info("World/Terrain", "Terrain instance created", {"name": terrain.name})

func configure_terrain() -> void:
	MythosLogger.verbose("World/Terrain", "configure_terrain() called")
	if not terrain:
		MythosLogger.warn("World/Terrain", "Cannot configure terrain - terrain instance is null")
		return

	terrain.vertex_spacing = 1.0
	terrain.region_size = 1024
	MythosLogger.debug("World/Terrain", "Terrain configured", {
	...
	if data_directory:
		...
		MythosLogger.debug("World/Terrain", "Data directory set", {"path": data_directory})

	if assets_resource:
		terrain.assets = assets_resource
		MythosLogger.debug("World/Terrain", "Assets resource assigned")

func generate_initial_terrain() -> void:
	MythosLogger.verbose("World/Terrain", "generate_initial_terrain() called")
	if terrain:
		...
		if ResourceLoader.exists("res://assets/heightmap.png"):
			...
			if img:
				MythosLogger.info("World/Terrain", "Generating terrain from heightmap image", {"size": img.get_size()})
				terrain.create_from_heightmap_image(img)
			else:
				MythosLogger.warn("World/Terrain", "Failed to load heightmap image")
		else:
			MythosLogger.debug("World/Terrain", "Heightmap image not found at res://assets/heightmap.png")
	else:
		MythosLogger.warn("World/Terrain", "Cannot generate initial terrain - terrain instance is null")
...
func generate_from_noise(seed_value: int, frequency: float, min_height: float, max_height: float) -> void:
	...
	MythosLogger.verbose("World/Terrain", "generate_from_noise() called", {
		"seed": seed_value,
		"frequency": frequency,
	...
	if not terrain:
		MythosLogger.warn("World/Terrain", "No terrain instance available")
		return
	...
	MythosLogger.debug("World/Terrain", "Noise generator configured", {"type": "Perlin"})
	
	# Generate heightmap image
	var size: int = 2048  # Heightmap resolution
	MythosLogger.verbose("World/Terrain", "Creating heightmap image", {"size": size})
	var height_image: Image = Image.create(size, size, false, Image.FORMAT_RF)
	
	# Sample noise into heightmap
	MythosLogger.verbose("World/Terrain", "Sampling noise into heightmap")
	for x in range(size):
		for y in range(size):
	...
	MythosLogger.debug("World/Terrain", "Heightmap image generated")
	...
	if terrain.has_method("data") and terrain.data != null:
		MythosLogger.verbose("World/Terrain", "Importing heightmap via terrain.data.import_images()")
		terrain.data.import_images([height_image, null, null], Vector3(-size/2, 0, -size/2), min_height, max_height)
		terrain.update_maps()
		MythosLogger.info("World/Terrain", "Terrain generated from noise", {
			"seed": seed_value,
			"frequency": frequency,
			"size": size,
		...
		if terrain.has_method("create_from_heightmap_image"):
			MythosLogger.debug("World/Terrain", "Using fallback create_from_heightmap_image() method")
			terrain.create_from_heightmap_image(height_image)
		else:
			MythosLogger.error("World/Terrain", "No valid method to import heightmap into terrain")
...
func generate_from_heightmap(heightmap_image: Image, min_height: float = -50.0, max_height: float = 300.0, terrain_position: Vector3 = Vector3.ZERO) -> void:
	...
	MythosLogger.verbose("World/Terrain", "generate_from_heightmap() called", {
		"min_height": min_height,
		"max_height": max_height,
	...
	MythosLogger.debug("World/Terrain", "Processing heightmap image", {"size": image_size})
	
	# Ensure image is in correct format (Terrain3D prefers 16-bit or float)
	if heightmap_image.get_format() != Image.FORMAT_RF:
		MythosLogger.verbose("World/Terrain", "Converting image format to RF")
	...
	if terrain.has_method("data") and terrain.data != null:
		...
		MythosLogger.info("World/Terrain", "Generated terrain from hand-drawn heightmap", {
			"size": image_size,
			"height_range": [min_height, max_height],
			"position": terrain_position
		})
	else:
		MythosLogger.error("World/Terrain", "Terrain data is null, cannot import heightmap")
